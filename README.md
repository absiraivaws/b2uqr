...
- **Webhook mode**: When PHPPOS posts sale data directly, the body can be either the sale object itself or `{ "sale": { ... } }`. As soon as the token matches, the payload is stored under `phppos_sales` and linked to any `transactionUuid`/`referenceNumber` you send along.
